...
function RecapCompareSection() {
   ...
   useEffect(() => {
     ...
     async function load() {
         ...
         for (const module of ASSESSMENT_MODULES) {
             const history = await getModuleHistory(module.id);

             if (history.length >= 2) {
                 results.push({ module, history });
             }
         }
 ...
 return (
     <div className="mt-8">
       ...
       <div className="space-y-6">
          {modulesWithHistory.map(({ module, history }) => {
            const latest = history[0];
            const previous = history[1];

            const scaleQuestions = module.questions.filter(
               (question: any) => question.type === "scale"
            );
            ...
            const chartData = scaleQuestions
               .map((question: any) => {
                   ...
                   )?.answer as { value?: number } | undefined;
                 ...
                 const previousValue =
                     typeof previousAnswer?.value === "number"
                       ? previousAnswer.value
                       : null;

                 const latestValue =
                     typeof latestAnswer?.value === "number"
                       ? latestAnswer.value
                       : null;
